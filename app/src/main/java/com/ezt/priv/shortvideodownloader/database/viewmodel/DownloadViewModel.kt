...
import com.ezt.priv.shortvideodownloader.database.models.expand.table.AudioPreferences
...
import com.ezt.priv.shortvideodownloader.database.models.expand.table.VideoPreferences
...
import com.ezt.priv.shortvideodownloader.ui.downloadcard.MultipleItemFormatTuple
import com.ezt.priv.shortvideodownloader.util.Extensions.needsDataUpdating
import com.ezt.priv.shortvideodownloader.util.Extensions.toListString
...
import com.ezt.priv.shortvideodownloader.work.AlarmScheduler
import com.ezt.priv.shortvideodownloader.work.UpdateMultipleDownloadsDataWorker
import com.ezt.priv.shortvideodownloader.work.UpdateMultipleDownloadsFormatsWorker
...
class DownloadViewModel(private val application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        historyRepository = HistoryRepository2(dbManager.historyDao, dbManager.segmentedVideoDao)
    ...
    fun createDownloadItemFromResult(result: ResultItem?, url: String = "", givenType: Type) : DownloadItem {
        ...
        val downloadPath = when(type){
            Type.audio -> sharedPreferences.getString("music_path", FileUtil.getDefaultAudioPath(application))
            Type.video -> sharedPreferences.getString("video_path",  FileUtil.getDefaultVideoPath(application))
            else -> sharedPreferences.getString("command_path", FileUtil.getDefaultCommandPath(application))
        }
    ...
    fun switchDownloadType(list: List<DownloadItem>, type: Type) : List<DownloadItem>{
        ...
        list.forEach {
            ...
            when(type){
                Type.audio -> {
                    updatedDownloadPath = sharedPreferences.getString("music_path", FileUtil.getDefaultAudioPath(application))!!
                    container = sharedPreferences.getString("audio_format", "")!!
                }
                Type.video -> {
                    updatedDownloadPath = sharedPreferences.getString("video_path", FileUtil.getDefaultVideoPath(application))!!
                    container = sharedPreferences.getString("video_format", "")!!
                }
                Type.command -> {
                    updatedDownloadPath = sharedPreferences.getString("command_path", FileUtil.getDefaultCommandPath(application))!!
                    container = ""
                }
                else -> {
                    updatedDownloadPath = ""
    ...
    fun createDownloadItemFromHistory(historyItem: HistoryItem) : DownloadItem {
        ...
        val defaultPath = when(historyItem.type) {
            Type.audio -> FileUtil.getDefaultAudioPath(application)
            Type.video -> FileUtil.getDefaultVideoPath(application)
            Type.command -> FileUtil.getDefaultCommandPath(application)
            else -> ""
        }
    ...
    suspend fun continueUpdatingFormatsOnBackground(selectedItems: List<Long>?){
        ...
        println("continueUpdatingFormatsOnBackground 0")
