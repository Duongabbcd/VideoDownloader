...
import com.ezt.priv.shortvideodownloader.util.Extensions.convertToTimestamp
import com.ezt.priv.shortvideodownloader.util.Extensions.setTextAndRecalculateWidth
import com.ezt.priv.shortvideodownloader.util.Extensions.toStringDuration
import com.ezt.priv.shortvideodownloader.util.Extensions.toStringTimeStamp
import com.ezt.priv.shortvideodownloader.util.Extensions.tryConvertToTimestamp
import com.ezt.priv.shortvideodownloader.util.UiUtil
import com.ezt.priv.shortvideodownloader.util.VideoPlayerUtil
...
//        }
    ...
    private fun populateSuggestedChapters() {
        if (chapters!!.isEmpty()) suggestedChapters.visibility = View.GONE
            ...
            chapters!!.forEach {
                val chip =
                    layoutInflater.inflate(R.layout.suggestion_chip, suggestedChips, false) as Chip
                chip.text = it.title
                ...
                if (idx > -1) {
                    val chipForDeletion =
                        chipGroup.children.firstOrNull { cc -> (cc as Chip).text.contains(it.title) }
    ...
    private fun createChapterChip(chapter: ChapterItem, position: Int?): Chip {
        val chip = layoutInflater.inflate(R.layout.filter_chip, chipGroup, false) as Chip
        val timestamp = "${chapter.start_time.toInt().toStringDuration(Locale.US)}-${
            chapter.end_time.toInt().toStringDuration(Locale.US)
        } [${chapter.title}]"
        chip.text = timestamp
        chip.chipBackgroundColor = ColorStateList.valueOf(
            MaterialColors.getColor(
    ...
    private fun cleanUp() {
        kotlin.runCatching {
            ...
            parentFragmentManager.beginTransaction()
                .remove(parentFragmentManager.findFragmentByTag("cutVideoSheet")!!).commit()
        }
    }
}
