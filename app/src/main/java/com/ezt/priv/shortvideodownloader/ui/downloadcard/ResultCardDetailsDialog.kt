...
import com.ezt.priv.shortvideodownloader.ui.adapter.ActiveDownloadMinifiedAdapter
import com.ezt.priv.shortvideodownloader.ui.adapter.GenericDownloadAdapter
import com.ezt.priv.shortvideodownloader.util.Extensions.setFullScreen
...
import com.ezt.priv.shortvideodownloader.work.DownloadWorker
                ...
                if (urls.size == 2){
                    val audioSource : MediaSource =
                        DefaultMediaSourceFactory(requireContext())
                            .createMediaSource(MediaItem.fromUri(urls[0].toUri()))
                    val videoSource: MediaSource =
                        DefaultMediaSourceFactory(requireContext())
                            .createMediaSource(MediaItem.fromUri(urls[1].toUri()))
                    player.setMediaSource(MergingMediaSource(videoSource, audioSource))
                }else{
                    player.addMediaItem(MediaItem.fromUri(urls[0].toUri()))
                }
