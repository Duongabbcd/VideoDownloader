...
import com.ezt.priv.shortvideodownloader.receiver.CancelDownloadNotificationReceiver
import com.ezt.priv.shortvideodownloader.receiver.CancelWorkReceiver
import com.ezt.priv.shortvideodownloader.receiver.PauseDownloadNotificationReceiver
import com.ezt.priv.shortvideodownloader.receiver.activities.ResumeActivity
...
import com.ezt.priv.shortvideodownloader.util.Extensions.toBitmap
            ...
            try{
                ...
                if (uris.isNotEmpty()){
                    openFileIntent.apply {
                        ...
                        putExtra("isFragmented", title?.contains("dailymotion", true))
                    }
...
//                notificationBuilder.addAction(0, res.getString(R.string.share), shareNotificationPendingIntent)
    ...
    fun updateDownloadNotification(
        ...
        title: String?,
        channel: String,
        segmentDao: SegmentedVideoDao,
    ) {
        println("updateDownloadNotification 1: $desc")
        if (totalFrag == null) {
            val extracted = extractFragTotal(desc)
            if (extracted != null) {
                totalFrag = extracted
                CoroutineScope(Dispatchers.IO).launch {
                    segmentDao.insert(SegmentedVideo(title = title ?: "", totalSegments = extracted))
                }
                println("FOUND TOTAL FRAGMENTS: $totalFrag")
            }
        }
    ...
    fun extractFragTotal(text: String): Int? {
        val regex = Regex("""frag\s+\d+\/(\d+)""")
    ...
    fun showFormatsUpdatedNotification(downloadIds: List<Long>) {
        val notificationBuilder = getBuilder(DOWNLOAD_FINISHED_CHANNEL_ID)
        println("continueUpdatingFormatsOnBackground 1")
        val bundle = Bundle()
        ...
        val activityIntent = Intent(context, FacebookInfoActivity::class.java).apply {
            putExtras(bundle)
