package com.ezt.video.downloader.database.viewmodel
...
class CommandTemplateViewModel(private val application: Application) : AndroidViewModel(application) {
    ...
    val sortOrder = MutableLiveData(VideoDownloadDB.SORTING.DESC)
    ...
    init {
        val dao = VideoDownloadDB.getInstance(application).commandTemplateDao
    ...
    fun setSorting(sort: CommandTemplateRepository.CommandTemplateSortType){
        if (sortType.value != sort){
            sortOrder.value = VideoDownloadDB.SORTING.DESC
        }else{
            sortOrder.value = if (sortOrder.value == VideoDownloadDB.SORTING.DESC) {
                VideoDownloadDB.SORTING.ASC
            } else VideoDownloadDB.SORTING.DESC
        }
        sortType.value = sort
    }

    private fun filter(query : String, sortType: CommandTemplateRepository.CommandTemplateSortType, sort: VideoDownloadDB.SORTING) = viewModelScope.launch(Dispatchers.IO){
